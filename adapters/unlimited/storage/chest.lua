...
function Chest:gatherDetails(itemName, slotIndex)
...
function Chest:update()
	...
	self:resetInfo()
	self.totalCapacity = p.size() * CHEST_MAX_SLOT_CAPACITY
...
function Chest:getSlot(itemName)
...
function Chest:pushItem(to, itemName, itemCount)
	local p = peripheral.wrap(self.name)
	if not p then
