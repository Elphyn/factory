...
local function is_exception(exn)
	if type(exn) ~= "table" then
		return false
	end

	local mt = getmetatable(exn)
	return mt
		and mt.__name == "exception"
		and type(rawget(exn, "message")) == "string"
		and type(rawget(exn, "thread")) == "thread"
end
...
local function can_wrap_errors(thread)
	...
	for offset = 0, 31 do
		...
		if func == try_barrier then
			...
			if type(parent) ~= "table" or type(parent.co) ~= "thread" then
				return false
			end
...
local function report(err, thread, source_map)
	...
	if not frame or not frame.currentcolumn then
		return
	end
	...
	if not line_contents or #line_contents == "" then
		return
	end
